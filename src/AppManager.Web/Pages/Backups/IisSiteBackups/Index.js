$(function () {
    ...
    var dt = $('#IisSiteBackupsTable').DataTable(abp.libs.datatables.normalizeConfiguration({
        serverSide: true, processing: true, paging: true, ordering: true, searching: false, scrollX: true,
        ajax: abp.libs.datatables.createAjax(appManager.application.backups.iisSiteBackup.getList),
        columnDefs: [
            { title: l('IisSites:SiteName'), data: 'siteName' },
    ...
    $('#IisSiteBackupsTable').on('click', '.delete-btn', function () {
        var id = $(this).data('id');
        abp.message.confirm(l('DeleteConfirmation'), function (c) {
            if (c) {
                appManager.application.backups.iisSiteBackup.delete(id).then(function () {
                    dt.ajax.reload();
                }).catch(function () {
                    dt.ajax.reload();
                });
